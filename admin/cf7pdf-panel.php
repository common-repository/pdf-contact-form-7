    ...
    class cf7pdf_panel {
        ...
        function cf7pdf_download_pdf(){
            if(isset($_REQUEST['action']) && $_REQUEST['action'] == "pdf_callback"){
                ...
                $dompdf->render();
                $dompdf->stream( $insert_id.'.pdf', array("Attachment" => false));
            }
        }
        ...
        function send_mail_attchment($components, $form, $object) {
            ...
            if(get_option('attach_pdf', 'attachs_pdf') == 'attachs_pdf') {  
                $form_id      = $form->id;
                ...
                if($send_pdf == "on"){
                    ...
                    $components['attachments'] = array($upload_file['file']);
                }
                return $components;
            }
            elseif(get_option('attach_pdf', 'attachs_pdf') == 'attach_custom_pdf'){ 

                $attach_pdf = get_option('attach_id');          
               
                $components['attachments'] = array(get_attached_file( $attach_pdf ));
                return $components;
            }elseif(get_option('attach_pdf', 'attachs_pdf') == 'boths_pdf'){
                $attach_pdf = get_option('attach_id');          
                ...
                $components['attachments'] = array(get_attached_file( $attach_pdf),$upload_file['file']);
